...
from recompute_ttr_from_bags import (  # noqa: E402
    binned_recovery,
    evaluate_window,
    interp_gt,
    log_bin_edges,
...
class TestAngles:
    ...
    def test_yaw_from_quat_pure_z(self):
        # 90 degrees about z
        assert abs(yaw_from_quat(0, 0, math.sin(0.25 * math.pi),
                                 math.cos(0.25 * math.pi)) - 0.5 * math.pi) < 1e-12
...
class TestEvaluateWindow:
    ...
    def test_touch_and_drift_is_not_recovered(self):
        ...
        gt = flat_gt(0.0, 20.0)
        series = (amcl_at([1.0, 2.0, 3.0], x=5.0)
    ...
    def test_sustained_entry_is_final_entry_not_first_touch(self):
        ...
        gt = flat_gt(0.0, 20.0)
        series = (amcl_at([1.0, 2.0], x=5.0) + amcl_at([4.0, 5.0])
