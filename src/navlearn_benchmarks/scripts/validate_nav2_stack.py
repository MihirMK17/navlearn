...
def _workspace_root():
    """Return the workspace root inferred from this script's location in the source tree."""
    here = os.path.dirname(os.path.abspath(__file__))
    return os.path.abspath(os.path.join(here, "..", "..", ".."))


def _default_stack_dir():
    """Locate config/nav2_stack/ in the source tree relative to this script."""
    return os.path.join(
...
def _load_stack_spec():
    ...
    candidates.append(
        os.path.join(
            _workspace_root(), "src", "bumperbot_navigation", "launch", "stack_spec.py"
        )
    )
