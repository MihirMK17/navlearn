...
"""Phase 3 vx_max single-variable ablation analyzer.

Cells (6): mppi_baseline_high_vx x extreme x {ttc, ttr} x seeds {42, 43, 44}
...
Verdict logic:
  If high_vx ~ aggressive AND high_vx >> baseline -> vx_max DOMINANT.
  If high_vx ~ baseline   AND high_vx << aggressive -> vx_max NOT dominant.
  Else PARTIAL.

Writes JSON to results/phase_comparison/ablation_vx_analysis.json.
"""
...
from compare_phase2_phase3 import (  # type: ignore  # noqa: E402
    _extract_wide_rows,
    cell_stats,
    fisher_p,
    wilson_ci,
...
ABL_DIR = WS / "results" / "phase3_ablation_vx"
OUT_PATH = WS / "results" / "phase_comparison" / "ablation_vx_analysis.json"
...
def load_cell(mode: str, seed: int) -> Dict[str, object]:
    ...
    cell_dir = ABL_DIR / f"mppi_baseline_high_vx_{mode}_extreme_seed{seed}"
...
def verdict(
    ...
    # vs aggressive: ns and high pct -> behaves like aggressive
    near_agg = (p_vs_agg >= 0.05) and (high_vx_pct >= agg_pct - 15)
    # vs baseline: significantly different + much higher
    diff_from_base = (p_vs_base < 0.05) and (high_vx_pct >= base_pct + 15)
    if near_agg and diff_from_base:
        return "DOMINANT"
    if (not diff_from_base) and (high_vx_pct < base_pct + 10):
        return "NOT_DOMINANT"
    return "PARTIAL"
...
def main() -> int:
    ...
    result: Dict[str, object] = {
        "benchmark": BENCHMARK,
        "ablation_profile": "mppi_baseline_high_vx",
        "single_variable_changed": "vx_max: 0.30 -> 0.45",
        "per_mode": {},
    }
    ...
    print("=" * 76)
    print(
        "Phase 3 vx_max Ablation — mppi_baseline_high_vx (= mppi_baseline + vx_max 0.45)"
    )
    print("=" * 76)
    for mode in MODES:
        ...
        for ps in r["per_seed"]:
            print(
                f"  seed={ps['seed']}: n={ps['n']} succ={ps['n_succ']} pct={ps['pct']:5.1f}%  "
                f"ate_mean={ps.get('ate_mean', float('nan')):7.2f}  nav_mean={ps.get('nav_mean', float('nan')):6.2f}"  # noqa: E501
            )
        p = r["pooled"]
        print(
            f"  POOLED: n={p['n']} succ={p['n_succ']} pct={p['pct']:5.1f}% "
            f"[Wilson {100*p['wilson_lo']:4.1f}%, {100*p['wilson_hi']:4.1f}%]"
        )
        b = BENCHMARK[mode]["baseline"]
        a = BENCHMARK[mode]["aggressive"]
        print(
            f"  vs mppi_baseline ({b['pct']:.1f}%):   Fisher p = {r['fisher_vs_baseline']:.4f}"
        )
        print(
            f"  vs mppi_aggressive ({a['pct']:.1f}%): Fisher p = {r['fisher_vs_aggressive']:.4f}"
        )
