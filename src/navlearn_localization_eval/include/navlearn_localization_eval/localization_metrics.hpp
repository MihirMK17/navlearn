...
private:
    ...
    void write_csv(std::string_view metric_class, std::string_view metric_source, 
    ...
    PoseStatistics fillPoseStatistics(const geometry_msgs::msg::PoseWithCovarianceStamped &pose, const rclcpp::Time &t);
    ...
    TFStatistics fillTFStatistics(const geometry_msgs::msg::TransformStamped &transform, const rclcpp::Time &t);
    boost::unordered_map<std::string, double> getLocalizationStability(const TFStatistics prev, const TFStatistics curr);
    ...
    bool linear_interpolate(const std::deque<TFStatistics>& q, TFStatistics &out, rclcpp::Time t_req, double max_gap_s, double max_stale_s);
