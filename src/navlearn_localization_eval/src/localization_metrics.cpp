...
LocalizationMetrics::LocalizationMetrics(const std::string & name)
: Node(name)
  ...
  , ttr_attempted_(false)
{
  ...
  ground_truth_tf_topic_ = this->declare_parameter<std::string>(
    "ground_truth_tf_topic",
    "/bumperbot/ground_truth_pose");
  initial_pose_topic = this->declare_parameter<std::string>(
    "initial_pose_topic",
    "/navlearn/bad_initialpose_event");
  ...
  if (bad_init_test_ && kidnap_test_) {
    RCLCPP_FATAL(
      get_logger(),
      "Cannot have bad initial pose and kidnap robot conditions true at the same time");
  ...
  if (csv_.tellp() > 0) {
    header_written_ = true;                             // Avoiding rewriting of header

  }
  RCLCPP_INFO(get_logger(), "AMCLEval writing to %s", csv_path_.c_str());

  episode_event_sub_ = create_subscription<navlearn_msgs::msg::EpisodeEvent>(
    episode_topic, 10,
  ...
  ground_truth_tf_sub_ = create_subscription<geometry_msgs::msg::TransformStamped>(
    ground_truth_tf_topic_, 10,
  ...
  if (kidnap_test_) {
    kidnap_event_sub_ = create_subscription<navlearn_msgs::msg::KidnapEvent>(
      kidnap_topic_, 10,
  ...
  timer_ =
    ...
    std::bind(&LocalizationMetrics::timerCallback, this));
}
...
void LocalizationMetrics::trim(
  std::deque<rclcpp::Time> & q, const rclcpp::Time & now,
  double max_buffer)
{
  while (!q.empty() && (now - q.front()).seconds() > max_buffer) {q.pop_front();}
}

void LocalizationMetrics::write_csv(
  ...
  if (episode_event.state == navlearn_msgs::msg::EpisodeEvent::START) {
    ...
    if (bad_init_test_) {
      if (ttc_arm_for_next_goal_) {
        ...
        RCLCPP_WARN(
          get_logger(), "Goal started but TTC was NOT armed (no initialpose between goals).");
      }
    }
    ...
    nav_time_ = static_cast<double>(episode_event.nav_time.sec) + 1e-9 *
      static_cast<double>(episode_event.nav_time.nanosec);
    ...
    if (bad_init_test_) {
      ...
      write_csv(
        "Localization Responsiveness", "TTC",
    ...
    write_csv(
      "Mission Outcome", "Episode Event", boost::unordered_map<std::string, double> {
    ...
    write_csv(
      "Input Validity", "AMCLPose", boost::unordered_map<std::string, double> {
    ...
    write_csv(
      "Input Validity", "Odometry", boost::unordered_map<std::string, double> {
        {"Odometry Monotonicity", odom_monotonicity_violations_},
        {"Odometry Frame Mismatch", odom_frame_mismatch_count_}
      }, t);

    write_csv(
      "Input Validity", "TF", boost::unordered_map<std::string, double> {
...
LocalizationMetrics::PoseStatistics LocalizationMetrics::fillPoseStatistics(
  const geometry_msgs::msg::PoseWithCovarianceStamped & pose, const rclcpp::Time & t)
{
...
void LocalizationMetrics::onAMCLPose(
  const geometry_msgs::msg::PoseWithCovarianceStamped & amcl_pose)
{
  ...
  curr_pose_ = fillPoseStatistics(amcl_pose, t);
  write_csv(
    "Localization Quality", "AMCLPose", boost::unordered_map<std::string, double> {
  ...
  if (bad_init_test_) {
    ...
    if (ttc_state_ == TtcState::MEASURING) {
      ...
      RCLCPP_INFO(
        get_logger(), "TTC error: ErrorX: %f || ErrorY: %f || ErrorYaw: %f", pose_error_x, pose_error_y,
        pose_error_yaw);

      if (std::fabs(pose_error_x) < error_x_threshold_ &&
        std::fabs(pose_error_y) < error_y_threshold_ &&
        std::fabs(pose_error_yaw) < error_yaw_threshold_)
      {
        ttc_state_ = TtcState::CONVERGED;
  ...
  if (ttr_state_ == TtrState::RECOVERED && !ok) {
    ...
    RCLCPP_INFO(
      get_logger(), "TTR: RECOVERED -> RECOVERING (threshold broken) at t=%.3f",
      t.seconds());
    return;
  }
...
LocalizationMetrics::TFStatistics LocalizationMetrics::fillTFStatistics(
  const geometry_msgs::msg::TransformStamped & transform, const rclcpp::Time & t)
{
...
boost::unordered_map<std::string, double> LocalizationMetrics::getLocalizationStability(
  const TFStatistics prev, const TFStatistics curr)
{
  const double jump_lin =
    std::sqrt(
    std::pow(
      (curr.transform_x - prev.transform_x),
      2) + std::pow((curr.transform_y - prev.transform_y), 2));
...
void LocalizationMetrics::onTF(const tf2_msgs::msg::TFMessage::SharedPtr transforms)
{
  ...
  for (const auto & transform : transforms->transforms) {
    if (transform.header.frame_id == tf_parent_frame_ &&
    ...
    } else if (transform.header.frame_id == tf_parent_frame_ &&
      transform.child_frame_id != tf_child_frame_)
    {
...
bool LocalizationMetrics::linear_interpolate(
  const std::deque<TFStatistics> & q, TFStatistics & out,
  rclcpp::Time t_req, double max_gap_s,
  double max_stale_s)
{
  ...
  for (size_t i = n; i-- > 0; ) {
    ...
    if (i + 1 < n) {
      ...
      const int64_t interp_ns = static_cast<int64_t>(std::llround(
          alpha * static_cast<double>(dt_ns)));
...
void LocalizationMetrics::trim(
  std::deque<TFStatistics> & q, const rclcpp::Time & now,
  double max_buffer)
{
...
void LocalizationMetrics::onInitialPose(
...
void LocalizationMetrics::onKidnapEvent(const navlearn_msgs::msg::KidnapEvent & kidnap_event)
{
  ...
  if (kidnap_event.success) {
    ...
    kidnap_angle_ =
      std::fabs(
      wrap_to_pi(
        ttr_stats_.goal_start_yaw_ -
        getYaw(kidnap_event.kidnap_pose.orientation)));

    RCLCPP_INFO(
      get_logger(), "TTR: IDLE -> KIDNAPPED at t=%.3f (dist=%.2f m, angle=%.2f rad)",
...
void LocalizationMetrics::timerCallback()
{
  ...
  if ((ttc_state_ == TtcState::MEASURING || ttc_state_ == TtcState::CONVERGED) && !ttc_done_) {
    if ((t - ttc_start_time_).seconds() > ttc_timeout_sec_) {
      RCLCPP_WARN(
        get_logger(), "TTC: MEASURING -> TIMED_OUT elapsed=%.3f",
        (t - ttc_start_time_).seconds());
    ...
    !ttr_done_ && ttr_outcome_ != 4)
  {
    if ((t - ttr_start_time_).seconds() > ttr_timeout_sec_) {
      RCLCPP_WARN(
        get_logger(), "TTR: RECOVERING -> TIMED_OUT elapsed=%.3f",
        (t - ttr_start_time_).seconds());
...
int main(int argc, char ** argv)
{
  ...
  auto node = std::make_shared<navlearn_localization_metrics::LocalizationMetrics>(
