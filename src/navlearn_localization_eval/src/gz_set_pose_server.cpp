...
GzSetPoseServer::GzSetPoseServer(const std::string &name) : Node(name)
{
    world_name_ = this->declare_parameter<std::string>("world_name", "default");
    ign_service_ = this->declare_parameter<std::string>("ign_service", "/world/" + world_name_ + "/set_pose");
    timeout_ms_ = this->declare_parameter<int>("timeout_ms", 2000);

    srv_ = create_service<navlearn_msgs::srv::SetEntityPose>("/gz_set_pose_server/set_entity_pose",
