...
GroundTruthTF::GroundTruthTF(const std::string &name) : Node(name)
{
    ...
    gazebo_groudnd_truth_topic_ = this->declare_parameter<std::string>("gazebo_ground_truth_topic", "/tf_gt");
    robot_ground_truth_topic_ = this->declare_parameter<std::string>("robot_ground_truth_topic", "/" + robot_frame_id_ + "/ground_truth_pose");

    gazebo_gt_tf_sub_ = create_subscription<tf2_msgs::msg::TFMessage>(gazebo_groudnd_truth_topic_, 10, 
        std::bind(&GroundTruthTF::onGazeboGT, this, _1));
    robot_gt_tf_pub_ = create_publisher<geometry_msgs::msg::TransformStamped>(robot_ground_truth_topic_, 10);

    tf_broadcaster_ = std::make_shared<tf2_ros::TransformBroadcaster>(this);

    RCLCPP_INFO(get_logger(), "Publishing Ground Truth Robot Pose to topic: %s", robot_ground_truth_topic_.c_str());
}
