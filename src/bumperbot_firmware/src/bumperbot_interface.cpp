...
hardware_interface::return_type BumperbotInterface::write(const rclcpp::Time & time, const rclcpp::Duration & period)
{
    ...
    std::string compenstate_zeros_right = "";
    std::string compenstate_zeros_left = "";

    if(std::abs(velocity_commands_.at(0) < 10.0))
    {
        compenstate_zeros_right = "0";
    }
    else{
        compenstate_zeros_right = "";
    }

    if(std::abs(velocity_commands_.at(1) < 10.0))
    {
        compenstate_zeros_left = "0";
    }
    else{
        compenstate_zeros_left = "";
    }

    message_stream << std::fixed << std::setprecision(2) << "r" << right_wheel_sign << compenstate_zeros_right << std::abs(velocity_commands_.at(0)) <<
    ",l" << left_wheel_sign << compenstate_zeros_left << std::abs(velocity_commands_.at(1)) << ",";
