...
from navlearn_analysis.harness import run_forensics, run_watchdog
            ...
            (
                subprocess.Popen(
                    [
                        sys.executable,
                        "-m",
                        "navlearn_analysis.probes.compute_sampler",
                        "--output-prefix",
            ...
            (
                subprocess.Popen(
                    [
                        sys.executable,
                        "-m",
                        "navlearn_analysis.probes.costmap_corruption_monitor",
                        "--output-prefix",
            ...
            (
                subprocess.Popen(
                    [
                        sys.executable,
                        "-m",
                        "navlearn_analysis.probes.rate_monitor",
                        "--output-prefix",
...
def main() -> None:
    ...
    # The campaign convention is to run from the workspace root; the SHA is of the
    # repository enclosing the CWD, found by walking up to .git rather than by counting
    # path components from this file, which silently broke whenever the file moved.
    repo = pathlib.Path.cwd().resolve()
    while repo != repo.parent and not (repo / ".git").exists():
        repo = repo.parent
    args.git_sha = git_sha(repo)
