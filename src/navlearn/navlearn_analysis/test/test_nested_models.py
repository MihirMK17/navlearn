...
from navlearn_analysis.nested_models import (  # noqa: E402
    compare_nested,
    cross_validated_auc,
    fit_logistic,
    roc_auc,
...
def test_cross_validated_auc_of_pure_noise_is_near_chance():
    ...
    assert cross_validated_auc(noise, y, folds=5, seed=1) == pytest.approx(
        0.5, abs=0.12
    )
...
def test_likelihood_ratio_test_is_calibrated_under_the_null():
    ...
    for seed in range(200):
        ...
        noise = rng.normal(size=300)
        p_values.append(
            compare_nested(y, {"distance": noise})["tests"]["M0->M1"]["p_value"]
        )

    false_positive_rate = float(np.mean(np.array(p_values) < 0.05))
    assert (
        0.01 <= false_positive_rate <= 0.12
