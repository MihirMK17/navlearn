...
from navlearn_analysis.map_ambiguity import (  # noqa: E402
    AmbiguityField,
...
def _carved(width_m, height_m, rects, res=RES):
    ...
    for x0, y0, x1, y1 in rects:
        occupied[
            int(round(y0 / res)) : int(round(y1 / res)),
            int(round(x0 / res)) : int(round(x1 / res)),
        ] = False
    return occupied
...
def _field(occupied, **kwargs):
    ...
    params = dict(
        pose_spacing_m=0.5, yaw_bins=8, n_beams=36, max_range_m=6.0, sigma_m=0.15
    )
...
def test_adding_a_landmark_reduces_ambiguity():
    ...
    assert _field(notched, max_range_m=2.0).ambiguity(*query) < _field(
        plain, max_range_m=2.0
    ).ambiguity(*query)
...
def _lf(occupied, **kwargs):
    ...
    params.update(kwargs)
    return LikelihoodFieldAmbiguity(
        occupied, resolution=RES, origin=(0.0, 0.0), **params
    )
...
def test_lf_all_max_range_beams_carry_no_information():
    ...
    assert result["entropy_bits"] == pytest.approx(
        math.log2(len(field.poses)), abs=0.01
    )
...
def test_lf_local_ambiguity_only_considers_nearby_hypotheses():
    ...
    assert result["n_hypotheses"] == int(
        np.sum(np.hypot(field.poses[:, 0] - 10.0, field.poses[:, 1] - 10.0) <= 2.0)
    )
...
def _campaign_map():
    """Path to the map the campaign actually runs on, via the ament index."""
    try:
        ...
        pytest.skip("bumperbot_mapping share directory not resolvable")
...
def test_campaign_map_loads_with_declared_geometry():
    ...
    assert not (
        occ.free & occ.occupied
    ).any(), "a cell cannot be both free and occupied"
