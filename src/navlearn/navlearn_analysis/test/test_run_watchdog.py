...
def wd():
    from navlearn_analysis.harness import run_watchdog
...
def test_first_goal_gets_the_startup_budget_not_the_goal_budget(wd):
    ...
    clock = FakeClock()
    watchdog = _make(
        wd, clock, progress=lambda: 0, goal_timeout_s=120.0, startup_timeout_s=300.0
    )
...
def test_a_dead_stack_process_aborts_immediately(wd):
    ...
    clock = FakeClock()
    watchdog = _make(wd, clock, progress=lambda: 1, liveness=lambda: ["planner_server"])
...
def test_supervise_kills_a_hung_run_and_reports_why(wd):
    ...
    verdict = wd.supervise(process, AlwaysStalled(), poll_interval_s=0.05, grace_s=2.0)
...
def test_supervise_captures_state_before_it_kills_the_launch(wd):
    ...
    wd.supervise(
        process, Stalled(), poll_interval_s=0.05, grace_s=2.0, on_abort=on_abort
    )
...
def test_supervise_kills_the_launch_even_if_the_forensic_hook_raises(wd):
    ...
    verdict = wd.supervise(
        process, Stalled(), poll_interval_s=0.05, grace_s=2.0, on_abort=explode
    )
...
def test_harness_exposes_both_watchdog_budgets(harness, monkeypatch):
    ...
    args = _args(
        harness, monkeypatch, "--goal-timeout", "42", "--startup-timeout", "99"
    )
...
def test_harness_watchdog_defaults_clear_the_slowest_observed_goal(
    harness, monkeypatch
):
...
def test_launch_supervised_reports_the_exit_code_of_a_healthy_run(
    harness, wd, tmp_path
):
    ...
    with open(log, "w") as handle:
        code, abort = harness.launch_supervised(
            ["true"], handle, Never(), poll_interval_s=0.05
        )
...
def test_launch_supervised_kills_a_hung_launch_and_spares_the_harness(
    ...
    with open(log, "w") as handle:
        code, abort = harness.launch_supervised(
            ["sleep", "300"], handle, Stalled(), poll_interval_s=0.05
        )
