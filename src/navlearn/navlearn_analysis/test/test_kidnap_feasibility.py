...
from navlearn_analysis.kidnap_feasibility import (  # noqa: E402
    feasible_fraction,
    valid_pose_mask,
)  # noqa: E402
...
def test_clearance_mask_excludes_cells_too_close_to_a_wall():
    ...
    assert not mask[
        :edge, :
    ].any(), "cells within the clearance radius of a wall passed"
...
def test_widening_the_band_never_reduces_feasibility():
    ...
    for tighter, wider in zip(fractions, fractions[1:]):
        assert (
            wider >= tighter - 1e-12
        ), f"widening the band lost feasibility: {fractions}"
...
def test_sparse_ring_is_reported_as_likely_missed_not_merely_possible():
    ...
    assert (
        realistic["expected_applied_fraction"] <= generous["expected_applied_fraction"]
    )
    assert (
        realistic["expected_applied_fraction"] <= realistic["feasible_fraction"] + 1e-12
    )
