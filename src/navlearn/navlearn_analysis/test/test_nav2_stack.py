...
def _navigation_share():
    ...
    return get_package_share_directory("bumperbot_navigation")


REAL_STACK = os.path.join(_navigation_share(), "config", "nav2_stack")
...
def stack_spec():
    ...
    Loaded from the installed share directory by path, exactly as the launch files
    load it — it is a launch-dir module, not an importable package.
    """
    return _load(
        "stack_spec",
        os.path.join(_navigation_share(), "launch", "stack_spec.py"),
    )
...
def run_validator(stack_dir):
    ...
    result = subprocess.run(
        [
            sys.executable,
            "-m",
            "navlearn_analysis.stack.validate_nav2_stack",
            "--stack-dir",
            stack_dir,
        ],
        capture_output=True,
...
def test_controllers_share_one_success_criterion(stack_spec):
    ...
    assert (
        len(tolerances) == 1
    ), f"success criterion differs across controllers: {tolerances}"
...
def test_catches_mppi_inflation_mismatch(temp_stack):
    ...
    with open(path, "w") as handle:
        handle.write(
            text.replace("cost_scaling_factor: 3.0", "cost_scaling_factor: 10.0", 1)
        )
...
def test_catches_velocity_desync(temp_stack):
    ...
    with open(path, "w") as handle:
        handle.write(
            text.replace("desired_linear_vel: 0.30", "desired_linear_vel: 0.26", 1)
        )
...
def test_stack_spec_records_fragments_and_identity(stack_spec):
    ...
    assert (
        spec["identity"]["controller_plugin"] == "nav2_mppi_controller::MPPIController"
    )
...
def test_stack_spec_changes_when_configuration_changes(stack_spec, temp_stack):
    ...
    with open(path, "w") as handle:
        handle.write(
            text.replace("xy_goal_tolerance: 0.05", "xy_goal_tolerance: 0.07", 1)
        )
...
def sampler():
    ...
    from navlearn_analysis.probes import compute_sampler

    return compute_sampler
...
def test_interpreter_hosted_nodes_are_resolved_by_cmdline(sampler):
    ...
    assert sampler.classify(
        "python3", "python3 /opt/ros/humble/lib/episode_manager"
    ) == ("MEASUREMENT")
