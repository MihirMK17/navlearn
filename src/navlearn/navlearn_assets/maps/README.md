...
```bash
ros2 run navlearn_analysis map_spread_gate \
    --map small_house=src/bumperbot/bumperbot_mapping/maps/small_house/map.yaml \
    --map small_warehouse=src/navlearn/navlearn_assets/maps/small_warehouse/map.yaml \
    --map bookstore=src/navlearn/navlearn_assets/maps/bookstore/map.yaml \
    --out results/map_spread_gate
```

## Worlds

`bookstore.world` and its 28 retail models are vendored in THIS package (`worlds/`,
`models/`), registered on `GZ_SIM_RESOURCE_PATH` by an ament environment hook that
`gazebo.launch.py`'s world search consumes — the robot stack needs no knowledge of this
package. `small_house.world` and `small_warehouse.world` predate the campaign and remain
with the robot in `bumperbot_description/worlds`.
