...
// NAVLEARN REGRESSION, added 2026-08-01. Not upstream.
//
// On 2026-08-01 planner_server took SIGSEGV mid-campaign. The core shows createPlan
// discarding the return value of worldToMap for the start pose, then handing the
// untouched sentinel values to setStart:
//
//   costmap: size 500x500, resolution 0.05, origin (-12.5, -12.5)   -> map spans +/-12.5 m
//   start.pose.position = (2.6226, -15.7264)                        -> 3.23 m outside it
//   mx_start, my_start  = 4294967295, 4294965088                    -> never written
//   Costmap2D::getCost(4293863295) on a 250000-cell array           -> out-of-bounds read
//
// A pose estimate outside the map is a normal outcome of a kidnap, so the planner has to
// refuse it rather than index on it. This test fails by killing the test binary if the
// bounds check is ever removed.
TEST(SmacTest, test_smac_2d_start_outside_map_bounds_is_refused_not_indexed) {
  rclcpp_lifecycle::LifecycleNode::SharedPtr node2D =
    std::make_shared<rclcpp_lifecycle::LifecycleNode>("Smac2DBoundsTest");

  std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros =
    std::make_shared<nav2_costmap_2d::Costmap2DROS>("global_costmap");
  costmap_ros->on_configure(rclcpp_lifecycle::State());

  node2D->declare_parameter("test.downsample_costmap", false);
  node2D->set_parameter(rclcpp::Parameter("test.downsample_costmap", false));
  ...
  goal.pose.orientation.w = 1.0;
  start.pose.position.x = -50.0;   // far outside any default costmap
  start.pose.position.y = -50.0;
  start.pose.orientation.w = 1.0;

  EXPECT_THROW(planner_2d->createPlan(start, goal), nav2_core::PlannerException);

  // The goal is checked on the same path and for the same reason.
  start.pose.position.x = 1.0;
  start.pose.position.y = 1.0;
  goal.pose.position.x = -50.0;
  goal.pose.position.y = -50.0;
  EXPECT_THROW(planner_2d->createPlan(start, goal), nav2_core::PlannerException);
