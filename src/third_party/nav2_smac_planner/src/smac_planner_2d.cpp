  ...
  const geometry_msgs::msg::PoseStamped & goal)
{
  ...
  // NAVLEARN PATCH (2026-08-01), not upstream. See ../../README.md.
  //
  // worldToMap returns false and leaves its output parameters UNWRITTEN when the pose is
  // off-map. Upstream discarded the return value here, so an off-map pose handed setStart
  // uninitialised stack values; A* then validated that node and Costmap2D::getCost indexed
  // 4293863295 into a 250000-cell array. That killed planner_server mid-campaign and took
  // the whole navigation stack down with it.
  //
  // A pose estimate outside the map is a normal outcome of a kidnap, so this must be
  // refused rather than indexed on. PlannerServer::computePlan catches std::exception and
  // aborts the action, which lets the behaviour tree run its recovery. Later distros raise
  // nav2_core::StartOutsideMapBounds; that type does not exist in Humble.
  ...
  if (!costmap->worldToMap(goal.pose.position.x, goal.pose.position.y, mx_goal, my_goal)) {
    throw nav2_core::PlannerException(
            "Goal pose (" + std::to_string(goal.pose.position.x) + ", " +
            std::to_string(goal.pose.position.y) + ") is outside the costmap bounds");
  }
